...
class RequestProcessor:
    ...
    def eval(self):#对延迟和吞吐进行测评
        ...
        output ={"batch_size":self.args.max_batch_size,"request_speed":self.args.Request_speed,"average_latency":all_latency/len(self.completed_stack),"throughput":all_count/(self.end_time-self.start_time),"Accept_ratio":accept_ratio/len(self.completed_stack),"TPOT_ratio":float(count/len(self.completed_stack)),"TPOT_list":TPOT_list}
...
if __name__ == "__main__":
    ...
    args.add_argument("--model_path", type=str,
                        default="/data/home/weijh/models_weight/EMNLP/Medusa/medusa-v1.0-vicuna-7b-v1.5",
    ...
    args.add_argument("--data_path", type=str,
                        default="/data/home/weijh/zyh/emnlp/data/MIX/shuffled_file.jsonl",
                        help="Path to the evaluation data in JSON format.")
    args.add_argument("--save_path", type=str, default="/data/home/weijh/zyh/毕设/BM1.0/Baseline/SpeculativeDecoding/result/ouput.jsonl",
                        help="Directory to save the results.")
    args.add_argument("--max_batch_size", default=20, type=int,
                        help="max Batch size for request")
    args.add_argument("--Medusa_choice", default="full_node", type=str,
                        help="Number of nodes to be pruned.")
    ...
    if args.Medusa_choice == "full_node":
        Medusa_choices=vicuna_7b_stage2_64
    elif args.Medusa_choice == "Topone":
        Medusa_choices=Topone

    Medusa = Parallel_decoding(args.model_path,medusa_choices=Medusa_choices,Device_index=device_index)
